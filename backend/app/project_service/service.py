...
def create_project_role(
    ...
    existing = db.scalar(
        select(ProjectRole).where(
            func.lower(ProjectRole.name) == schema.name.strip().lower()
        )
    )
...
def update_project_role(
    ...
    if schema.name and schema.name.strip().lower() != role.name.lower():
        ...
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Another project role with this name exists."
            )
...
def update_project_status(
    ...
    if (
        old_status == ProjectStatus.PLANNED
    ...
    if old_status in (
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                f"{old_status} project."
            ),
        )

    if (
        new_status == ProjectStatus.COMPLETED
        and project.progress_percentage < 100
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Project progress must reach 100% "
    ...
    for assignment in active_assignments:
        ...
        if emp and emp.user_id:
            ...
            notif = Notification(
                ...
                message=(
                    f"The status of project "
                    f"'{project.name}' ({project.project_code}) "
                    f"has changed to {new_status.value}."
                ),
...
def assign_employee_to_project(
    ...
    if (
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Employee not found or is not currently ACTIVE."
            ),
        )

    role = db.get(
        ProjectRole,
        schema.project_role_id
    )
    ...
    if not role or not role.is_active:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Project role not found or is inactive."
            ),
        )
    ...
    if existing_active:
        raise HTTPException(
            ...
            detail=(
                ...
                f"{employee.last_name} is already actively "
    ...
    if employee.user_id:
        ...
        notif = Notification(
            ...
            message=(
                f"You have been assigned to project "
                f"'{project.name}' ({project.project_code}) "
                f"as {role.name}."
            ),
            notification_type=NotificationType.PROJECT_UPDATE,
    ...
    if employee.user and employee.user.email:
        try:
            ...
            # Get the Employee record belonging to the user
            # who assigned the project.
            #
            # User does not have "employee" or "username"
            # attributes. Employee.user_id is the existing
            ...
            assigned_by_name = (
                ...
                f"{assigned_by_employee.last_name}".strip()
            ...
            logger.error(
                f"Failed to send project assignment email "
                f"to {employee.user.email}: {e}"
            )
...
def remove_employee_from_project(
    ...
    if (
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Project assignment record not found."
        )

    if assignment.status == AssignmentStatus.REMOVED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Employee assignment is already removed."
        )
    ...
    if (
        ...
        notif = Notification(
            ...
            message=(
                f"You have been removed from project "
                f"'{project.name}' ({project.project_code})."
            ),
            notification_type=NotificationType.PROJECT_UPDATE,
            reference_id=str(project.id),
...
def get_project_team(
    ...
    stmt = (
        ...
        .order_by(
            ProjectAssignment.status.asc(),
            ProjectAssignment.assigned_date.desc()
        )
    )
...
def get_employee_my_projects(
    ...
    for a in assignments:
        ...
        result.append({
            ...
            "is_overdue": calculate_is_overdue(
                prj.end_date,
                prj.status
            ),
            "project_role_name": (
...
def get_employee_project_details(
    db: Session,
    employee_id: int,
    project_id: int
) -> dict:
    ...
    if not active_assignment:
        raise HTTPException(
            ...
            detail=(
                "Access denied. You are not an active "
                "member of this project."
            ),
        )

    prj = get_project_by_id(
        db,
        project_id
    )

    team = get_project_team(
        db,
        project_id
    )
    ...
    return {
        ...
        "my_role": (
            active_assignment.project_role.name
            if active_assignment.project_role
            else "Team Member"
        ),
